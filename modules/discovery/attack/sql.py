                ...
                for url in self.urls:
                    ...
                    if len(param) > 1:
                        for para in param:
                            ...
                            error_message = self.db_error(resp.content.decode('utf-8'))
                            if error_message is not None:
                                self.output.plus(f'That site is may be vulnerable to {error_message} at {para}')
                        ...
                        error_message = self.db_error(resp.content.decode('utf-8'))
                        if error_message is not None:
                            self.output.plus(f'That site is may be vulnerable to {error_message} at {param[0]}')
